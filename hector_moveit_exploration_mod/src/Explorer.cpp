...
void Quadrotor::landing()
{
    hector_uav_msgs::EnableMotors srv;
    srv.request.enable = false;
    while(!odom_received)
        ;
    geometry_msgs::Pose landing_pose = odometry_information;
    landing_pose.position.z = landing_altitude;
    go(landing_pose);
    motor_enable_service.call(srv);
...
void Quadrotor::gotocenter(const std::vector<double> &p1, const std::vector<double> &p2, const std::vector<double> &p3, const std::vector<double> &p4)
{
    ...
    double centerRecty;
    double centerRectz;
    centerRectx = p1[0] - p2[0];
    centerRecty = p2[1] - p3[1];
    centerRectz = 15.0;

    while(!odom_received)
        ;
    geometry_msgs::Pose gotocenter_pose;

    gotocenter_pose.position.x = centerRectx;
    gotocenter_pose.position.y = centerRecty;
    gotocenter_pose.position.z = centerRectz;
...
std::vector<std::vector<double>> Quadrotor::computewaypoint(const std::vector<std::vector<double>> &treematrix)
{
    std::vector<std::vector<double>> tempmatrix;
    tempmatrix = treematrix;
    ROS_INFO("computewaypoint successful");
    return tempmatrix;
}
